...
# Inspired from https://github.com/aroberge/reeborg-dist by André Roberge
# Adapted with the necessary changes.
#
# Minas Rousouliotis
...
def tracing_line(indent, current_group, last_line=False, skip_watch=False):
    ...
    tracecall_name0 = 'infiniteloop'
    tracecall_name = 'recframe'
    ...
    if last_line:
        return watch_info
    if _highlight and not skip_watch:
        trace = indent + tracecall_name0 + '%s' % current_group + '\n' + indent + tracecall_name + '%s' % current_group
    elif _highlight and  skip_watch:
        trace = indent + tracecall_name0 + '%s' % current_group + ' and' + indent+ tracecall_name + '%s' % current_group
    else:
        trace = ''
